...
% 配置参数
show_flow_labels = true;  % 是否显示流动权重标签
min_label_height = 0.02;  % 显示标签的最小流动带高度
...
for i = 1:n1
    for j = 1:n2
        if flow12(i, j) > 0
            ...
            % 添加权重标签
            if show_flow_labels && flow_height >= min_label_height
                mid_x = (x_positions(1) + node_width + x_positions(2)) / 2;
                mid_y = (y1_current(i) + y2_current_left(j)) / 2 + flow_height / 2;
                
                if show_percentage
                    percentage = flow12(i, j) / sum(flow12(:)) * 100;
                    label_text = sprintf('%d (%.1f%%)', flow12(i, j), percentage);
                else
                    label_text = sprintf('%d', flow12(i, j));
                end
                
                text(mid_x, mid_y, label_text, ...
                     'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
                     'FontSize', label_font_size, 'FontWeight', 'bold', ...
                     'Color', 'white', 'BackgroundColor', [0 0 0 0.3], ...
...
for i = 1:n2
    for j = 1:n3
        if flow23(i, j) > 0
            ...
            % 添加权重标签
            if show_flow_labels && flow_height >= min_label_height
                mid_x = (x_positions(2) + node_width + x_positions(3)) / 2;
                mid_y = (y2_current_right(i) + y3_current(j)) / 2 + flow_height / 2;
                
                if show_percentage
                    percentage = flow23(i, j) / sum(flow23(:)) * 100;
                    label_text = sprintf('%d (%.1f%%)', flow23(i, j), percentage);
                else
                    label_text = sprintf('%d', flow23(i, j));
                end
...
for i = 1:n1
    ...
    % 计算节点百分比
    node_percentage = stage1_totals(i) / sum(stage1_totals) * 100;
    text(x - 0.01, y + h/2, sprintf('%s: %d (%.1f%%)', stage1_labels{i}, stage1_totals(i), node_percentage), ...
...
for i = 1:n2
    ...
    % 计算节点百分比
    node_percentage = stage2_totals(i) / sum(stage2_totals) * 100;
    text(x + node_width/2, y + h + 0.02, sprintf('%s: %d (%.1f%%)', stage2_labels{i}, stage2_totals(i), node_percentage), ...
...
for i = 1:n3
    ...
    % 计算节点百分比
    node_percentage = stage3_totals(i) / sum(stage3_totals) * 100;
    text(x + node_width + 0.01, y + h/2, sprintf('%s: %d (%.1f%%)', stage3_labels{i}, stage3_totals(i), node_percentage), ...
...
% 添加总流量信息
total_flow = sum(stage1_totals);
text(0.5, 0.02, sprintf('总流量: %d', total_flow), ...
     'HorizontalAlignment', 'center', 'FontSize', 12, 'FontWeight', 'bold', ...
     'BackgroundColor', [1 1 1 0.8], 'EdgeColor', [0.3 0.3 0.3], 'LineWidth', 1);

% 设置坐标轴
axis([0 1 0 1]);
axis off;
title('加权冲击图 (Weighted Alluvial Diagram)', 'FontSize', 16, 'FontWeight', 'bold');

hold off;

% 保存图片
% saveas(gcf, '加权冲击图示例.png');
% fprintf('图片已保存为: 加权冲击图示例.png\n');
