...
for i = numGroups:-1:1
    ...
    % 获取当前密度曲线
    y = densities(i, :) + baseline;
    
    % 创建填充区域的顶点
    xFill = [xGrid, fliplr(xGrid)];
    yFill = [y, ones(1, smoothPoints) * baseline];
    
    % 填充区域
    fill(xFill, yFill, colors(i, :), ...
        'EdgeColor', 'none', ...
        'FaceAlpha', 0.9);
    
    % 绘制轮廓线
    plot(xGrid, y, 'Color', colors(i, :) * 0.6, 'LineWidth', 1.5);
...
% 设置坐标轴范围
xlim([xMin, xMax]);
ylim([-0.1, (numGroups - 1) * (1 - overlap) + 1.2]);

% 标签和标题
xlabel('数值', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('月份', 'FontSize', 12, 'FontWeight', 'bold');
title('山脊图 - 各月数据分布对比', 'FontSize', 14, 'FontWeight', 'bold');
