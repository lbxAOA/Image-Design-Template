...
% 随机生成网格参数
grid_size = randi([80, 120]);                   % 随机网格大小 (80-120) - 增加密度使曲面更圆滑
...
% 添加随机噪声使数据更真实
noise_level = 0.02 + rand() * 0.08;             % 随机噪声强度 (0.02-0.1) - 减少噪声使曲面更圆滑
...
%% 2. 创建图形窗口
figure('Name', '带等高线的3D曲面图', 'Position', [100, 100, 900, 700], 'Color', 'w');
...
%% 7. 设置标题
title('带等高线的3D曲面图示例', 'FontSize', 14, 'FontWeight', 'bold', 'FontName', 'Microsoft YaHei');
...
%% 11. 保存图形
% saveas(gcf, '带等高线的3D曲面图.png');
% saveas(gcf, '带等高线的3D曲面图.fig');
% print(gcf, '带等高线的3D曲面图', '-dpng', '-r300'); % 高分辨率保存
